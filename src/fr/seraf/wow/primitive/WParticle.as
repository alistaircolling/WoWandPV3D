...
package fr.seraf.wow.primitive {
	...
	public class WParticle {
		 ...
		 * @private
		 */
		public function WParticle(x : Number = 0, y : Number = 0, z : Number = 0, isFixed : Boolean = true, mass : Number = 1, elasticity : Number = 0.3, friction : Number = 0) {
		...
		public function set position(p : WVector) : void {
			if (!fixedY) {
				...
				curr.x = WVectorMath.clone(p).x;
				prev.x = WVectorMath.clone(p).x;
			}
		}
